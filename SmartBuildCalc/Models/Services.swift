...
protocol StorageService {
    ...
    func savePermissions(_ permission: SagaContext.PermissionData)
    func markLaunched()
...
final class UserDefaultsStorageService: StorageService {
    private let store = UserDefaults(suiteName: "group.smartbuild.data")!
    private let cache = UserDefaults.standard
    
    private enum Key {
        static let tracking = "sbc_tracking_payload"
        static let navigation = "sbc_navigation_payload"
        static let endpoint = "sbc_endpoint_target"
        static let mode = "sbc_mode_active"
        static let firstLaunch = "sbc_first_launch_flag"
        static let permGranted = "sbc_perm_granted"
        static let permDenied = "sbc_perm_denied"
        static let permDate = "sbc_perm_date"
    }
...
final class SupabaseValidationService: ValidationService {
    ...
    init() {
        self.client = SupabaseClient(
            supabaseURL: URL(string: "https://uadulistxqpzyrfejrup.supabase.co")!,
            supabaseKey: "sb_publishable_hm3B2OUraMLnxyKASkZ5xQ_USUrIqyM"
        )
    }
...
final class HTTPNetworkService: NetworkService {
    ...
    func fetchAttribution(deviceID: String) async throws -> [String: Any] {
        var builder = URLComponents(string: "https://gcdsdk.appsflyer.com/install_data/v4.0/id\(SmartBuildConfig.appID)")
        builder?.queryItems = [
            URLQueryItem(name: "devkey", value: SmartBuildConfig.devKey),
    ...
    func fetchEndpoint(tracking: [String: Any]) async throws -> String {
        guard let url = URL(string: "https://smartbuildcallc.com/config.php") else {
            throw NetworkError.invalidURL
        }
        ...
        payload["store_id"] = "id\(SmartBuildConfig.appID)"
        ...
        var lastError: Error?
        let retries: [Double] = [24.0, 48.0, 96.0]
...
struct SmartBuildConfig {
    static let appID = "6761608077"
    static let devKey = "cQT6kFBPUZecupAdQLg9tR"
}
