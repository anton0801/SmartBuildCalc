...
final class AppDelegate: UIResponder, UIApplicationDelegate, MessagingDelegate, UNUserNotificationCenterDelegate {
    ...
    func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String?) {
        messaging.token { token, error in
            ...
            UserDefaults(suiteName: "group.smartbuild.data")?.set(token, forKey: "shared_fcm")
        }
    }
...
final class AttributionBridge: NSObject {
    ...
    func receiveNavigation(_ data: [AnyHashable: Any]) {
        guard !UserDefaults.standard.bool(forKey: "sbc_first_launch_flag") else { return }
        navigationBuf = data
        onNavigation?(data)
        timer?.invalidate()
...
final class SDKBridge: NSObject, AppsFlyerLibDelegate, DeepLinkDelegate {
    ...
    func configure() {
        ...
        sdk.appsFlyerDevKey = SmartBuildConfig.devKey
        sdk.appleAppID = SmartBuildConfig.appID
        sdk.delegate = self
